 ...
 * @author javert
 */
public class SensorInventoryController extends Controller<SensorInventoryView, Integer> {

    public SensorInventoryController(SensorInventoryView view, Integer model) {
        super(view, model);
    }
    ...
    public void init() {
        ...
        for (Map.Entry<Integer, DeviceInventoryItem> entry : Worker.getInstance().SENSOR_INVENTORY.entrySet()) {
            if (Objects.equals(entry.getKey(), this.model)) {
                Logger.getInstance().log(new LMessage("\n"), true);

                Device device;
                if ((device = Worker.getInstance().SENSORS.get(this.model)) != null) {
